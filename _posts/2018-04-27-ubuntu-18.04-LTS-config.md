...
把程序移动到/usr/bin/目录下，再把配置文件移动到/etc/overture目录下

```sh
sudo mv ./overture-linux-amd64 /usr/bin/overture
cd ..
sudo mv ./overture /etc/
```

配置overture服务

```sh
sudo vim /lib/systemd/system/overture.service
```
...
Ubuntu 18.04 LTS 默认使用systemd-resolved进行dns解析，停掉该服务，再开启overture
...
如果被占用，就杀掉进程(一般被ubuntu自带的dnsmasq占用)
